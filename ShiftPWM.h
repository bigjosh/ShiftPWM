...
// busyFlag make sure that we do not re-enter when load gets high. Re-entrance would be bad since we 
// would step all over ourselves. 
...
static inline void ShiftPWM_handleInterrupt(void){
    ...
    // If this function is defined in cpp files of the library, it is compiled separately from the main file.
